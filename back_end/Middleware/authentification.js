...
export async function protectAdmin (req, res, next){
    ...
    try {
        ...
        const user  = await Utilisateur.findById(decoded.id);
        if(!user){
            return next(res.status(404).json({success: false, error: "No admin found with this id"}));
        }
        if(user.role !="admin"){
            return next(res.status(404).json({success: false, error: "you're not an admin"}));
        }
        req.user = user;//set that user on the request object to use it in other routes
        next(); //continue on the next piece in the route
    } catch (error) {
        return next(res.status(401).json({success: false, error: "Only admins can access this route"}));
